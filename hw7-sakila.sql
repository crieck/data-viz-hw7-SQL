...
-- 0a.  Installed sakila database, now need to use.
...
UPDATE actor SET actor_name = CONCAT(first_name,' ', last_name), actor_name = UPPER(actor_name)
	WHERE actor_id <> 0;
...
SELECT last_name, COUNT(*) AS number_of_last_names FROM actor
	GROUP BY last_name
    HAVING COUNT(*)>1
	ORDER BY 2 DESC;
...
UPDATE actor
    ...
    WHERE (actor_name = 'GROUCHO WILLIAMS' AND actor_id <> 0);
...
CREATE TABLE IF NOT EXISTS copy_address (
...
SELECT CONCAT(first_name, ' ',last_name) AS staff_name, SUM(amount) AS august_sales FROM staff
...
SELECT title, COUNT(*) AS number_of_film_actors FROM film
	INNER JOIN film_actor ON film.film_id = film_actor.film_id
    GROUP BY title;

-- 6d.  How many copies of the film `Hunchback Impossible` exist in the inventory system?

SELECT title, COUNT(inventory_id) AS copies FROM inventory
...
SELECT first_name, last_name, SUM(amount) AS customer_total FROM customer
...
SELECT title, NAME AS lang FROM film 
